...
async function initData() {
    try {
        ...
        const [proveedores] = await query('SELECT COUNT(*) as count FROM proveedores');
        ...
        await query(`
            ...
            VALUES ('Administrador', 'admin@ferreteria.com', ?, 'admin', 'activo')
            ON DUPLICATE KEY UPDATE contrasena = VALUES(contrasena)
        `, [adminPassword]);
        
        await query(`
            INSERT INTO usuarios (nombre, correo, contrasena, rol, estado) 
            VALUES ('Empleado Demo', 'empleado@ferreteria.com', ?, 'empleado', 'activo')
            ON DUPLICATE KEY UPDATE contrasena = VALUES(contrasena)
        `, [empleadoPassword]);

        const [productos] = await query('SELECT COUNT(*) as count FROM productos');
